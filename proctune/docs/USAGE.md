...
## 一·五、算法定位（这套系统用的是什么方法）

| 维度 | 说明 |
|------|------|
| **框架统称** | 通用「**受控工艺参数推荐**」（Controlled Process Parameter Recommendation） |
| **核心算法** | **贝叶斯优化 BO**（自实现 **GP + UCB**：高斯过程代理模型 + 上置信界采集函数） |
| **模型层** | 代理模型（Surrogate）+ 质量模型 + 推荐引擎（**BO + 离散枚举**） |
| **问题本质** | 黑盒 / 灰盒工艺参数寻优（质量函数无解析式，从数据学规律再反求最优旋钮） |
| **工业别名** | 工艺参数优化、参数寻优、实验设计（DOE）、代理模型辅助优化、序贯调参 |

**为什么选贝叶斯优化（BO）？** 工艺"参数→质量"的关系没有显式公式、评估一次成本高（要真产一批），
正是 BO 的典型场景：用**代理模型**拟合已知样本，用**采集函数（UCB）**在"探索未知区"与"利用已知优区"间权衡，
每次只挑最有价值的参数点，样本利用率远高于网格 / 随机搜索。

- **连续旋钮**（回流焊 / 注塑）→ 走 BO 分支：`core/models` 内 **GP + UCB** 序贯寻优。
- **离散旋钮**（黄铜高炉选炉）→ 走**离散枚举择优**：直接对候选集用质量模型打分排序。
- 两条路径对上层引擎 `ProcessTuningEngine` 透明，由 `BusinessProfile` 的旋钮类型自动分派。

> 落地增强项（见文末对比表）：代理模型可换 PyTorch PINN（物理约束网络）、
> 推荐搜索可换 `bayesian-optimization` 等成熟库，接口不变。
